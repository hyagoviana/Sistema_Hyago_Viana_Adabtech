...
| 2026-08-07 | v0.3 | CORREÇÃO (owner via Orion). Revisada a decisão: em vez de POP-UP, agora é PÁGINA dedicada. Ao clicar num tema na Pipeline Operacional navega para `/pipeline?cat=..&picker=true&temaId=..` → nova `KanbanPickerPage` (cards dos kanbans com os FUNIS listados EM TÓPICOS + o bloco de notas/links do tema — C5 — embaixo de tudo). Aparece SEMPRE, mesmo com 1 kanban. Escolher um kanban navega para a esteira. `KanbanPickerDialog.tsx` (pop-up) REMOVIDO. Arquivos: novo `src/components/pipeline/KanbanPickerPage.tsx`; `pipeline.tsx` (search `picker`/`temaId` + roteamento + clique do tema); dialog deletado. typecheck limpo, eslint 0. | @dev |
