...
| **R2-04** | ✅ **Concluída** (dev+qa+architect; migration APLICADA) | Pastas/modelos/checklist **por frente**: `frente_slug` aditivo em `system_service_type_folders` e `system_stage_checklist_defs` (UNIQUEs com `COALESCE`), views recriadas, `system_fn_instanciar_checklist` filtra por frente (validada byte-a-byte). Filtro de pastas/modelos = `frente do caso OR NULL` + **fallback `case_type`** (casos sem frente não perdem modelos). UI de vincular pasta a uma frente (destrava R2-06 AC-3). 11 casos intactos. |

| **R2-05** | ✅ **Concluída** (dev+qa+architect; sem migration) | Cadastro do caso escolhe **TEMA→FRENTE** (selects encadeados); servidor autoritativo resolve o service_type interno do tema e grava **dual-write** (`case_type`/`service_type_id`) **+ `tema_id`/`frente_slug`**. `case_code` deriva do **nome do TEMA** (só casos novos). Kanban com **filtro de frente** (oculta colunas condicionais vazias, mantém as com caso; auto-avanço intocado). Lista com coluna Frente. Coexistência aditiva com categorias legadas. AC-5 soft-delete diferido→R2-02. |

**🎉 FLUXO MANUAL DO R2 COMPLETO** (R2-01/03/04/05/06): admin cria tema→frentes→vincula pastas/modelos; pipeline nasce semeada; operador cria caso por tema+frente (puxa docs/checklist da frente); case_code por tema. | **R2-07** | ✅ **Concluída** (dev+qa+architect; migration APLICADA) | Estrutura de **campos personalizados por tema/frente**: `system_tema_field_defs` (tema_id, frente_slug NULL=painel do tema, key/label/type/options/required/ordem; UNIQUE `COALESCE`). Editor admin no `TemasManagerDialog` (gate admin server-side). `CaseCanonicalFields` renderiza as defs do tema+frente do caso (por tipo) **preservando valores livres remanescentes** (AC-4). Valor segue em `canonical_fields` (S2-07 inalterado). Fronteira A2/R5: campos FIES concretos ficam no R5-06; aqui é a estrutura genérica. 11 casos intactos. |

| **R2-08** | ✅ **Concluída** (dev+qa; sem migration) | UX: **toggle Kanban↔Lista** (search params `cat`/`tema`/`frente`, round-trip lossless); **Lista "Excel"** (11 colunas densas: +Tema/Frente/etapa fin/responsáveis/criado em) com **ordenação por coluna** (sobre o filtrado completo antes de paginar, `PAGE_SIZE=50` mantido); filtro de frente na Lista; **coluna valor sob gate financeiro** (não vaza $); visibilidade `getVisibleCaseIds` preservada. |

| **R2-02** | ✅ **Concluída (redefinida = construção manual)** (dev+qa+architect) | Decisão do dono: **não fundir automaticamente**. (a) **5 temas fictícios** (Tema 1..5) criados via `scripts/seed-temas-manuais.ts` (cada um com service_type interno + pipeline), editáveis na UI. (b) **Vincular caso existente a um tema** (`moverCasoParaTema` + `LinkCaseToTemaDialog` na ficha; gate casos.manage): reatribui case_type/tema_id/frente_slug + `service_type_id=null` p/ o trigger reprojetar (bug de `stage_op_id` NULL pego pelo arquiteto e corrigido); preserva a etapa se existir no tema, senão reseta 1ª. (c) **Pasta do tema no Drive** (botão no editor de tema → `createAndLinkFolder`). Sem migration; dual-write vivo. |

**🎉 R2 COMPLETO (8/8, modo manual)** — R2-01→R2-08 concluídas e pushadas. O dono cria/edita temas, vincula casos (novos e existentes), define frentes/campos/pastas — tudo manual. A fusão N:1 dos legados FIES foi substituída (por decisão do dono) pela construção manual. Smoke em runtime recomendado: vincular um caso de teste a um tema e conferir o board.

### Épico R1 — COMPLETO (5/5)
| **R1-04** | ✅ **Concluída** (dev+qa) | Ficha da pessoa **agrupada por TEMA** (nível 1) via adaptador `getCaseTemaKey` (usa `tema_id` real de R2, fallback `case_type`, grupo "Sem tema"); dentro reusa `partitionCasesByLifecycle` (R1-03). Rótulos amigáveis (useTemas/useServiceTypes, 1x). Client-side, sem query nova. Teste de contrato 12/12. |
| **R1-05** | ✅ **Concluída** (dev+qa) | **Vínculo pessoa↔caso** (por CASO, nunca na pessoa): bloco "Vínculo no caso" com município (coluna) + `vinculo_empregaticio`/`vinculo_papel` (`canonical_fields`, namespace `vinculo_*`, mesmo padrão FIES); filtrado do `CaseCanonicalFields`; autofill mapeia sob rótulo amigável. Merge por caso (vínculos independentes). Sem migration. |

**✅ Épico R1 (Modelo Pessoa/Lead/Cliente por caso) COMPLETO.** Reforma restante: **R2-02** (fusão legados, espera lista) · **R6/R7/R8** (design, dependem de ProIuris/mockups do cliente).
> Smoke em runtime recomendado (R2-05, com temas cadastrados): criar caso por tema → ver no board + case_code com nome do tema + docs da frente. Board por tema hoje via service_type interno (`?cat`); `?tema=` explícito + agregação de N service_types = R2-08/R2-02.
