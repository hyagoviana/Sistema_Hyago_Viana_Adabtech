...
**Próximo no R2:** R2-03 (pipeline op única por tema + seeding) e R2-04 (pastas/modelos por frente — `frente_slug` em `system_service_type_folders`) destravam o AC-3/AC-4 do R2-06; R2-02 (backfill) **precisa da lista do cliente**; R2-05 (criar caso escolhe tema+frente + case_code), R2-07 (campos por tema), R2-08 (Kanban+Lista). R1-04/R1-05 entram aqui.
> ⚠️ Ressalva do arq. (p/ R2-03/04): quando `service_type.tema_id` for populado, estender a guarda de exclusão de tema (`deleteTema`) para contar também casos via `service_types.tema_id` (hoje só conta `cases.tema_id`). Double-audit (insert manual + trigger) = limpeza futura.
