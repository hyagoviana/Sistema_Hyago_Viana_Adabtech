...
- **ID:** M13
- **Status:** Implementado (marcadores + URGENTE nativo + diag) · Pendente só T4b (seed eligible_complex dos 4 — depende dos ativos do M15)
...
| 2026-08-08 | v1.1 | **T3 URGENTE/PRIORITÁRIO nativo FEITO** (@aios-master/Orion). Migration `20260808000080` (`system_cases.distribution_urgency` CHECK prioritario|urgente|NULL) aplicada 2× + rollback. Service `setCaseUrgency` + RPC `setCaseUrgencyFn` (gate operacional:edit) + hook `useSetCaseUrgency`. UI: `<select>` Normal/Prioritário/Urgente no header da ficha (só `podeGerirCaso`). **Motor:** `sync-core` consulta `system_cases` por `projuris_codigo_processo` → `urgencyByCode` → `Process.temporal_level` (prioritario=1/urgente=2); scoring aplica TEMPORAL_MODIFIER. Sem marca ⇒ 0 (sem regressão). Typecheck+lint verdes. Restou só T4b (seed `eligible_complex` dos 4 — precisa dos ATIVOS do M15, adiados pelo owner). | @aios-master (Orion) |
