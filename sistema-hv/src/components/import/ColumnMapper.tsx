...
export function ColumnMapper({ headers, sampleRows, targetEntity, mappings, onChange, extraFields = [] }: Props) {
  const availableFields = useMemo(() => {
    const base = targetEntity === "client+case"
      ? TARGET_FIELDS
    ...
    <TooltipProvider delayDuration={300}>
      <div className="space-y-4">
        ...
        <div className="flex items-start gap-2.5 text-xs text-muted-foreground">
          ...
          <p>
            Para cada coluna do seu arquivo, escolha o <strong className="text-[var(--navy)]">campo no sistema</strong> correspondente.
            Use <strong className="text-[var(--navy)]">transformacao</strong> se o formato precisa ser convertido (ex.: datas, CPF).
            {suggested > 0 && (
              <span className="text-amber-600"> Sugerimos {suggested} campos automaticamente (<Sparkles size={10} className="inline" />).</span>
            )}
          </p>
        </div>
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5">
                    <span className="text-[13px] font-medium text-[var(--navy)] truncate">{header}</span>
                    {wasSuggested && (
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <span className="shrink-0 cursor-default"><Sparkles size={11} className="text-amber-500" /></span>
                        </TooltipTrigger>
                        <TooltipContent side="right" align="center" className="text-xs max-w-[200px]">
                  ...
                  <div className="text-[11px] text-muted-foreground/70 truncate mt-0.5">
                    {samples.map((r) => r[header]).filter(Boolean).slice(0, 3).join(" · ") || "—"}
                  </div>
                </div>
                ...
                <div className="w-[220px] shrink-0">
                  <Select
                    ...
                    onValueChange={(v) => updateMapping(idx, "targetField", v === "__none__" ? "" : v)}
                  >
                    ...
                    <SelectContent>
                      ...
                      {Array.from(fieldsByGroup.entries()).map(([group, fields]) => (
                        <SelectGroup key={group}>
                          ...
                          {fields.map((f) => {
                            const typeLabel = f.fieldType === "multiselect"
                              ? "multipla escolha"
                              : f.fieldType === "select"
                                ? "escolha"
                                : f.fieldType === "boolean"
                            ...
                            return (
                              <SelectItem key={f.key} value={f.key}>
                                ...
                                {typeLabel && (
                                  <span className="text-muted-foreground ml-1.5 text-[10px]">({typeLabel})</span>
                                )}
                              </SelectItem>
                            );
                          })}
              ...
              {isMultiselect && (
                <div className="px-3 py-1.5 text-[11px] text-purple-700 bg-purple-50/50 border border-t-0 border-purple-200 rounded-b-lg">
                  Campo de multipla escolha — na planilha, separe os valores com <strong>;</strong> (ex: <code className="bg-purple-100 px-1 rounded">valor1; valor2; valor3</code>). A transformacao ja foi aplicada automaticamente.
                </div>
              )}
              </div>
            );
