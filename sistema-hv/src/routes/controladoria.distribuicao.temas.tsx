...
function TemasPage() {
  ...
  const previewPoints = useMemo(() => (10 * parseFloat(multiplier || "1")).toFixed(2), [multiplier]);
  ...
  function openEdit(m: Record<string, unknown>) {
    setEditing(m); setProjurisCodigo(m.projuris_tema_codigo as string);
    setProjurisDesc((m.projuris_tema_descricao as string) ?? "");
    setMotorId(m.motor_theme_id as string); setMultiplier(String(m.multiplier));
    setTemporal(String(m.temporal_level)); setActive(m.active as boolean);
    setDialogOpen(true);
  }

  function openNew() {
    setEditing(null); setProjurisCodigo(""); setProjurisDesc(""); setMotorId("");
    setMultiplier("1.0"); setTemporal("0"); setActive(true); setDialogOpen(true);
  }
      ...
      {isLoading ? <Skeleton className="h-[300px]" /> : (
        <Card><CardContent className="pt-4">
          <table className="w-full text-sm">
            <thead><tr className="border-b"><th className="text-left py-2">Codigo Projuris</th><th className="text-left py-2">Motor ID</th><th className="text-right py-2">Multiplicador</th><th className="text-center py-2">Temporalidade</th><th className="text-center py-2">Status</th><th className="text-center py-2">Acoes</th></tr></thead>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          ...
          <div className="space-y-4">
            <div><Label>Codigo Projuris</Label><Input value={projurisCodigo} onChange={e => setProjurisCodigo(e.target.value)} disabled={!!editing} placeholder="TRABALHISTA" /></div>
            <div><Label>Descricao</Label><Input value={projurisDesc} onChange={e => setProjurisDesc(e.target.value)} placeholder="Trabalhista Reclamacao" /></div>
            <div><Label>Motor Theme ID</Label><Input value={motorId} onChange={e => setMotorId(e.target.value)} placeholder="trabalhista_reclamacao" /></div>
            <div>
              <Label>Multiplicador</Label>
              <Input type="number" min="0.5" max="3.0" step="0.1" value={multiplier} onChange={e => setMultiplier(e.target.value)} />
              <p className="text-xs text-muted-foreground mt-1">Preview: tipo com 10pts = <strong>{previewPoints}pts</strong> base</p>
            </div>
