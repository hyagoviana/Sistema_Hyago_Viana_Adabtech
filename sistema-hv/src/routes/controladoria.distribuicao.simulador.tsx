...
function SimuladorPage() {
  ...
  const filteredComparison = onlyDivergences
    ? comparison.filter((c) => c.match === false)
    : comparison;
  ...
  const executorComparison = useMemo(() => {
    ...
    for (const s of simResults ?? []) {
      if (!s.blocked && s.executor_id)
        simLoads[s.executor_id] = (simLoads[s.executor_id] ?? 0) + s.final_points;
    }
    for (const r of realResults ?? []) {
      if (!r.blocked && r.executor_id)
        realLoads[r.executor_id] = (realLoads[r.executor_id] ?? 0) + r.final_points;
    }
    const allExecs = [...new Set([...Object.keys(simLoads), ...Object.keys(realLoads)])];
    return allExecs
      .map((e) => ({ executor: e, sim: simLoads[e] ?? 0, real: realLoads[e] ?? 0 }))
  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex items-center gap-4">
        ...
        />
        {hasRealData && (
          <Badge className="bg-green-100 text-green-700">Dados reais disponiveis</Badge>
        )}
        ...
        <label className="flex items-center gap-1 text-sm cursor-pointer ml-auto">
          <input
            ...
            onChange={() => setOnlyDivergences(!onlyDivergences)}
          />
      ...
      {simLoading ? (
        ...
        <Card>
          <CardContent className="py-12 text-center text-muted-foreground">
            Nenhuma simulacao encontrada para {date}. Execute uma simulacao primeiro.
          </CardContent>
        </Card>
      ) : (
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <Card>
              <CardContent className="pt-4">
                ...
                <div className="text-xs text-muted-foreground">Simuladas</div>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="pt-4">
            ...
            <Card>
              <CardContent className="pt-4">
                ...
                {concordance !== null && (
                  <Badge
                    className={`text-xs mt-1 ${concordance >= 80 ? "bg-green-100 text-green-700" : "bg-amber-100 text-amber-700"}`}
                  >
                    {concordance >= 80 ? "Alta" : "Baixa"}
                  </Badge>
                )}
              </CardContent>
            </Card>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <Card className="md:col-span-2">
              <CardHeader>
                <CardTitle className="text-base">Comparacao por Tarefa</CardTitle>
              </CardHeader>
              <CardContent>
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b">
                      ...
                      <th className="text-center py-2">Motor</th>
                  ...
                  <tbody>
                    {filteredComparison.slice(0, 50).map((c) => (
                      <tr
                        key={c.task_id}
              ...
              <Card className="md:col-span-2">
                <CardHeader>
                  <CardTitle className="text-base flex items-center gap-2">
                    <Users className="h-4 w-4" /> Carga Motor vs Real
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="space-y-2">
                    {executorComparison.slice(0, 10).map(({ executor, sim, real }) => {
                      const max = Math.max(
                        ...executorComparison.map((e) => Math.max(e.sim, e.real)),
                        1,
                      );
                      return (
                        <div key={executor} className="space-y-1">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <div className="flex-1 bg-muted rounded-full h-2 relative">
                              <div
                                className="h-2 rounded-full bg-blue-500 absolute"
                                style={{ width: `${(sim / max) * 100}%` }}
                              />
                            </div>
                          </div>
                          <div className="flex items-center gap-2">
                            ...
                            <div className="flex-1 bg-muted rounded-full h-2 relative">
                              <div
                                className="h-2 rounded-full bg-green-500 absolute"
                    ...
                    <div className="flex gap-4 text-xs mt-2">
                      <span className="flex items-center gap-1">
                        <div className="w-3 h-3 rounded bg-blue-500" /> Motor
                      </span>
                      <span className="flex items-center gap-1">
                        <div className="w-3 h-3 rounded bg-green-500" /> Real
                      </span>
                    </div>
                  </div>
                </CardContent>
