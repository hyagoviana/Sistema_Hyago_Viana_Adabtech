...
import { useAlertsSummary30d, useExecutorMappings } from "@/hooks/useDistribuicao";
    ...
    <div className="space-y-6 p-6">
        ...
        <Card>
          <CardContent className="pt-4">
            {m90Loading ? (
              ...
              <div className="text-2xl font-bold">{m90?.toFixed(2) ?? "·"}</div>
            )}
          ...
          <CardContent className="pt-4">
            {devLoading ? (
              ...
              <div className="flex items-center gap-2">
                ...
                <Badge
                  className={`text-xs ${(loadDev ?? 0) < 15 ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
                >
                  {(loadDev ?? 0) < 15 ? "Meta" : "Acima"}
                </Badge>
              </div>
            )}
          ...
          <CardContent className="pt-4">
            {prefLoading ? (
              ...
              <div className="flex items-center gap-2">
                ...
                <Badge
                  className={`text-xs ${(prefRate ?? 0) >= 80 ? "bg-green-100 text-green-700" : "bg-amber-100 text-amber-700"}`}
                >
                  {(prefRate ?? 0) >= 80 ? "Meta" : "Abaixo"}
                </Badge>
              </div>
            )}
        ...
        <Card>
          <CardContent className="pt-4">
            <div className="flex items-center gap-2">
              ...
              <Badge
                className={`text-xs ${d0Count === 0 ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
              >
                {d0Count === 0 ? "Zero" : "Atencao"}
              </Badge>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base flex items-center gap-2">
              <Activity className="h-4 w-4" /> Producao Diaria ({period}d)
            </CardTitle>
          </CardHeader>
          <CardContent>
            {dailyTotals.length === 0 ? (
              <div className="text-sm text-muted-foreground py-4">Sem dados no periodo</div>
            ) : (
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base flex items-center gap-2">
              <Users className="h-4 w-4" /> Carga por Executor ({period}d)
            </CardTitle>
          </CardHeader>
          <CardContent>
            {executorTotals.length === 0 ? (
              <div className="text-sm text-muted-foreground py-4">Sem dados no periodo</div>
            ) : (
                ...
                {executorTotals.slice(0, 10).map(([execId, total]) => {
                  ...
                  return (
                    <div key={execId} className="flex items-center gap-2 text-xs">
                      <span className="w-24 truncate" title={execNameById.get(execId) ?? execId}>
                        {execNameById.get(execId) ?? `${execId.slice(0, 8)}…`}
                      </span>
                      <div className="flex-1 bg-muted rounded-full h-2">
                        <div
        ...
        <Card className="md:col-span-2">
          <CardHeader>
            <CardTitle className="text-base flex items-center gap-2">
              <AlertTriangle className="h-4 w-4 text-amber-500" /> Alertas por Tipo (30d)
            </CardTitle>
          </CardHeader>
          <CardContent>
            {alertsSummary && Object.keys(alertsSummary).length > 0 ? (
              ...
              <div className="text-sm text-muted-foreground py-4">
                Nenhum alerta nos ultimos 30 dias
              </div>
            )}
          </CardContent>
        </Card>
