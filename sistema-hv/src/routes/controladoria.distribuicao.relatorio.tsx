...
function RelatorioPage() {
  ...
  const simDays = useMemo(
    () => new Set((simData ?? []).map((s) => s.distribution_date)).size,
    [simData],
  );
  ...
  const concordance = useMemo(() => {
    ...
    for (const s of simData) {
      ...
      if (realExec) {
        total++;
        if (realExec === s.executor_id) matches++;
      }
    }
  ...
  function exportCSV() {
    const header =
      "Task ID;Processo;Data;Motor Executor;Motor Pontos;Motor Fluxo;Motor Data Final;Bloqueada;Alertas\n";
    const rows = (simData ?? [])
      .map(
        (s) =>
    ...
    a.href = url;
    a.download = `relatorio_simulacao_${startDate}_${endDate}.csv`;
    a.click();
  ...
  function generatePDFText() {
    ...
    const content = [
      ...
      sections.resumo
        ...
        : "",
      sections.concordancia
        ? [
            `== METRICAS DE CONCORDANCIA ==`,
            concordance !== null
              ? `Taxa de concordancia com o real: ${concordance}%`
              : `Dados reais nao disponiveis para comparacao`,
            ``,
          ].join("\n")
        : "",
      sections.carga
      ...
      sections.alertas
        ? [
            `== ALERTAS ==`,
            ...Object.entries(
              (simData ?? []).reduce((acc: Record<string, number>, s) => {
      ...
      sections.recomendacao && observations
        ? [`== OBSERVACOES DO CONTROLADOR ==`, observations, ``].join("\n")
        : "",
    ]
      .filter(Boolean)
      .join("\n");
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = `relatorio_validacao_${startDate}_${endDate}.txt`;
    a.click();
    URL.revokeObjectURL(url);
  }
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            <CardTitle className="text-base">Periodo e Secoes</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              {(Object.entries(sections) as [keyof typeof sections, boolean][]).map(
                ([key, checked]) => (
                    ...
                    {
                      {
                        ...
                        concordancia: "Metricas de Concordancia",
                        carga: "Analise de Carga",
                        alertas: "Alertas e Excecoes",
          ...
          <CardContent>
            {isLoading ? (
              ...
              <div className="space-y-2 text-sm">
                <div>
                  Dias de simulacao: <strong>{simDays}</strong>{" "}
                  {simDays < 5 && (
                ...
                <div>
                  Total tarefas: <strong>{totalSim}</strong>
                </div>
                <div>
                  Bloqueadas: <strong>{blockedSim}</strong>
                </div>
                <div>
                  Concordancia: <strong>{concordance !== null ? `${concordance}%` : "N/A"}</strong>
                </div>
              </div>
            )}
          </CardContent>
      ...
      {sections.recomendacao && (
        <Card>
          <CardHeader>
            <CardTitle className="text-base">Observacoes do Controlador</CardTitle>
          </CardHeader>
          <CardContent>
            <Textarea
              value={observations}
              onChange={(e) => setObservations(e.target.value)}
              placeholder="Insira observacoes, recomendacoes ou ressalvas antes de gerar o relatorio..."
              rows={4}
            />
          </CardContent>
        </Card>
      ...
      <div className="flex flex-wrap gap-3">
        <Button onClick={generatePDFText} disabled={!canGenerate}>
          <Download className="h-4 w-4 mr-1" /> Gerar Relatorio (TXT)
        </Button>
        <Button variant="outline" onClick={exportCSV} disabled={!totalSim}>
      ...
      {!canGenerate && (
        <p className="text-sm text-muted-foreground flex items-center gap-1">
          <AlertTriangle className="h-4 w-4" /> Necessarios pelo menos 5 dias de simulacao para
          gerar o relatorio.
        </p>
      )}
      ...
      <Dialog open={activateDialogOpen} onOpenChange={setActivateDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Esta acao ativa o Motor de Distribuicao em producao. A distribuicao manual deixara de
              ser necessaria. O proximo batch sera executado automaticamente no horario configurado.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 mt-2">
            <div className="p-3 bg-amber-50 border border-amber-200 rounded text-sm">
              <strong>Atencao:</strong> Certifique-se de que o periodo de simulacao foi validado e
              aprovado pelo Dr. Hyago antes de ativar.
            </div>
            <div className="flex gap-2 justify-end">
              <Button variant="outline" onClick={() => setActivateDialogOpen(false)}>
                Cancelar
              </Button>
              <Button variant="destructive" onClick={activateMotor} disabled={setActive.isPending}>
                {setActive.isPending ? "Ativando..." : "Confirmar Ativacao"}
              </Button>
            </div>
          </div>
        </DialogContent>
