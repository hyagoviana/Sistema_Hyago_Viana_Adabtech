...
import { scoreTask, type ScoredTask } from "./scoring";
import {
  calculateBaseDate,
...
import { buildGeneralBlockSet } from "./date-utils";
import { selectFlow, isAbsoluteExecutorEligible } from "./flow-selector";
import { assignResponsible, getEligibleExecutors } from "./responsible-engine";
import { generateAlerts, hasBlockingAlert, alertCodes, type Alert } from "./alerts";
 ...
 */
function sortTasksForProcessing(tasks: Task[], processes: Map<string, Process>): Task[] {
  return [...tasks].sort((a, b) => {
    ...
    const ta = Math.max(
      pa?.temporal_level ?? 0,
      a.task_type_temporal_level,
      a.task_override_temporal_level,
      a.theme_temporal_level,
    );
    const tb = Math.max(
      pb?.temporal_level ?? 0,
      b.task_type_temporal_level,
      b.task_override_temporal_level,
      b.theme_temporal_level,
    );
    ...
    const ca = Math.max(
      pa?.complexity_level ?? 0,
      a.task_type_complexity_level,
      a.task_override_complexity_level,
      a.theme_complexity_level,
    );
    const cb = Math.max(
      pb?.complexity_level ?? 0,
      b.task_type_complexity_level,
      b.task_override_complexity_level,
      b.theme_complexity_level,
    );
    // Absoluto vem antes de tudo, mas detectamos via directed/exclusive
    const hasAbsA = !!(
      pa?.directed_executor_id ||
      a.theme_exclusive_executor_id ||
      a.task_type_exclusive_executor_id
    );
    const hasAbsB = !!(
      pb?.directed_executor_id ||
      b.theme_exclusive_executor_id ||
      b.task_type_exclusive_executor_id
    );
 ...
 */
function distributeTask(task: Task, ctx: MotorContext): TaskResult {
  ...
  const { baseDate, alerts: baseAlerts } = calculateBaseDate(
    ctx.distributionDate,
    task.fatal_date,
    scored.final_temporal,
    flow === "ABSOLUTE",
    generalBlocks,
  );
  ...
  const updatedQueueState = ctx.queueState;
...
export function distributeBatch(input: BatchInput): BatchOutput {
  ...
  const ctx: MotorContext = {
    ...
    queueState: {
      ...frozen.queue_state,
      general_balances: { ...frozen.queue_state.general_balances },
      complex_balances: { ...frozen.queue_state.complex_balances },
      rotating_order: [...frozen.queue_state.rotating_order],
    },
    dayLoads: new Map(),
