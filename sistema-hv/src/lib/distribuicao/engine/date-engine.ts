...
import type { CalendarDay, OperationMode, TaskResult, TemporalLevel } from "./types";
import type { ScoredTask } from "./scoring";
import {
  addOperationalDays,
...
const DAILY_FACTORS: Record<OperationMode, Record<string, number>> = {
  HIGH_PRODUCTION: { monday: 0.95, tuesday_to_thursday: 1.0, friday: 0.9 },
  HIGH_CONTROL: { monday: 0.9, tuesday_to_thursday: 1.0, friday: 0.8 },
};
 ...
 */
export function calculateDailyReference(m90: number, date: string, mode: OperationMode): number {
...
function findNonUrgentDate(
  ...
  for (const day of eligibleDays) {
    ...
    const occ = calculateOccupancy(load, ref);
    const dist = Math.abs(
      eligibleDays.indexOf(day) - findClosestIndex(eligibleDays, preferredDate),
    );
